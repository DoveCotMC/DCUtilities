...
public class PlayerInteract {
    public static void BeaconPlacement(Plugin plugin, FileConfiguration config, Player player, Block block, EquipmentSlot hand) {
        ...
        if (usableBlockAmount >= 0) {
            if (total >= 1476) {
                tryReplaceBlockWithSpecified(plugin, player, world, inv, materials, oL);
            } else {
    ...
    private static void tryReplaceBlockWithSpecified(Plugin plugin, Player player, World world, PlayerInventory inv, Material[] materials, Location oL) {
        for (int y = 1; y <= 4; y++) {
            for (int x = -y; x <= y; x++) {
                for (int z = -y; z <= y; z++) {
                    ...
                    if (!(world.getBlockAt(toRemove).getType() == materials[0])) {
                        Bukkit.getScheduler().runTaskLater(plugin, () -> {
                            ...
                            final int itemToRemove = tryRemoveStackInInv(player, inv, materials[0], new int[]{0, 1}) * 9;
                            tryRemoveStackInInv(player, inv, materials[1], new int[]{0, itemToRemove});
                        }, 10L);
                    }
                }
            }
